...
!function (e, t) {
    ...
    const i = {
        ...
        B = r("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", L).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),
        C = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,
        ...
        P = r(/^!?\[(ref)\](?:\[\])?/).replace("ref", h).getRegex(), Z = {
            ...
            ...Z,
            link: r(/^!?\[(label)\]\((.*?)\)/).replace("label", C).getRegex(),
            reflink: r(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", C).getRegex()
        }, D = {
            ...Z,
            ...
            text: r(D.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex()
        }, O = {normal: x, gfm: w, pedantic: m}, Q = {normal: Z, gfm: D, breaks: M, pedantic: v},
        j = {"&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;"}, N = e => j[e];
